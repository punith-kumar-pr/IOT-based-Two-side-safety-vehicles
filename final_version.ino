...
void loop() {
  ...
  smsMessage = "Emergency, Accident occured. Location: https://maps.google.com/maps?q=loc:";
...
// ACCIDENT -----------------------------------------------------------
 if(digitalRead(tiltPin) == LOW || digitalRead(impactPin) == LOW){
    ...
    if(modem.sendSMS(SMS_TARGET, smsMessage+lati+","+longi)){
      SerialMon.println(smsMessage+lati+","+longi);
    } 
    else{
